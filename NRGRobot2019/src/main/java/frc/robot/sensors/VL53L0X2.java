 ...
 */
public class VL53L0X2 {
	public final static int VL53L0X_I2CADDR = 0x52;
	...
	public VL53L0X2(int address) {
		this(address, 0);
	}

	public VL53L0X2(int address, int timeout) {
		this.ioTimeout = timeout;

        this.vl53l0x = new I2C(I2C.Port.kOnboard, address);

		// Check identification registers for expected values.
        // From section 3.2 of the datasheet.
        byte[] id = new byte[3];

        boolean status = this.vl53l0x.read(0xC0, 3, id);
        System.out.println("status is " + status + ", device id is " + Arrays.toString(id));
		if (id[0] != 0xEE || id[1]!= 0xAA || id[2] != 0x10) {
		...
		byte[] refSpadMap = new byte[6];
		

        this.vl53l0x.read(GLOBAL_CONFIG_SPAD_ENABLES_REF_0, 6, refSpadMap);
        ...
        byte[] newSpadMap = new byte[7];
        newSpadMap[0] = (byte) GLOBAL_CONFIG_SPAD_ENABLES_REF_0;
        for(int i = 1; i < newSpadMap.length; ++i) {
            newSpadMap[i] = refSpadMap[i-1];
        }
		...
		for (int i = 0; i < 48; i++) {
			...
			if (i < firstSpadToEnable || spadsEnabled == spadInfo.count) {
				newSpadMap[1 + (int) (i / 8)] &= ~(1 << (i % 8));
			} else {
				spadsEnabled += 1;
			}
		}

		this.vl53l0x.writeBulk(newSpadMap);
	...
	private int readU16LE(int register) {
        ...
        vl53l0x.read(register, 2, data);
        return (data[0]&0x0ff) | ((data[1]&0x0ff)<<8);
	}

	private int readU16BE(int register) {
		byte[] data = new byte[2];
        vl53l0x.read(register, 2, data);
        return (data[1]&0x0ff) | ((data[0]&0x0ff)<<8);
	}

	private int readS16LE(int register) {
		byte[] data = new byte[2];
        vl53l0x.read(register, 2, data);
        return (data[0]&0x0ff) | ((data[1])<<8);
	}

	private byte[] readBlockData(int register, int nb) {
		byte[] data = new byte[nb];
		this.vl53l0x.read(register, nb, data);
		return data;
	}
